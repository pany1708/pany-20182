...
public class OrderServiceImpl implements OrderService
{
    ...
    @HystrixCommand(groupKey = "OrderGroup", commandKey = "CreateOrderCommand",fallbackMethod = "createOrderFallback",
            commandProperties = {
    ...
    private WebApiResponse<?> createOrderFallback(OrderDTO orderDTO,Throwable e){
        LOGGER.error("findAllMember 发生异常，进入熔断，异常信息：" + e.toString());
    ...
    public WebApiResponse<?> delOrder(String orderSn, String token) throws Exception {
        ...
        if (result > 0){
            ...
            //发送消息更新缓存
            String cacheKey= CacheKeysConstants.generateCacheKey(SingleOrderDTO.class, orderItem.getSn());
    ...
    public WebApiResponse<?> comment(OrderCommentDTO commentDTO) throws Exception {
        ...
        Member memberName = memberDubboService.selectByUuid(memberUuid);
        buyersShow.setMemberName(memberName == null ? "" : memberName.getUserName());
     ...
     */
    @Override
    public WebApiResponse<?> showSingleOrderInfo(String orderSn, String token) throws Exception {

        if (StringUtils.isEmpty(orderSn) || StringUtils.isEmpty(token)){
            return WebApiResponse.error(WebApiResponse.ResponseMsg.PARAMETER_ERROR.getValue());
        }
        ...
        var.setSn(orderSn);
        var.setMemberUuid(memberUuid);
        if (orderItemDubboService.selectOne(var) == null){
            return WebApiResponse.error(WebApiResponse.ResponseMsg.PARAMETER_ERROR.getValue());
        }

        String cacheKey= CacheKeysConstants.generateCacheKey(SingleOrderDTO.class, orderSn);
            ...
            if (expire <= 0) {
                ...
                try {
                    ...
                    Map<String, String> mapFigure = futureFigure.get()
                            .stream()
                            .collect(Collectors.toMap(Figure::getUuid, figure -> StringUtils.isEmpty(figure.getFigureName()) ? "" : figure.getFigureName() ));
                ...
                if (null == dto) {
                    ...
                    //加入缓存
                    redisManager.set(cacheKey, JSON.toJSONString(dto), TimeUnit.DAYS);
                }
