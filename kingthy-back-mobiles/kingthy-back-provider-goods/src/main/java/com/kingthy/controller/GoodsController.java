...
public class GoodsController
{
    ...
    public WebApiResponse queryGoods(
        @PathVariable @ApiParam(name = "goodsUuid", value = "商品的UUID", required = true) String goodsUuid,
        @RequestHeader("Authorization") String token)
    {
        ...
        try
        {
            ...
            if (StringUtils.isEmpty(redisManager.get(token)))
            {
    ...
    @Deprecated
//    @ApiOperation(value = "增量获取商品")
    @PostMapping("/queryGoods")
    public WebApiResponse queryGoodsIncrement(@RequestBody IncrementQueryReq incrementQueryReq)
    {
        PageInfo<GoodsIncrement> pageInfo;
        
        try
        {
            pageInfo = goodsService.queryGoodsIncrement(incrementQueryReq.getPageNum(),
                incrementQueryReq.getPageSize(),
                incrementQueryReq.getReferenceTime());
        }
        catch (Exception e)
        {
            LOG.error("/goods/queryGoods:" + e);
            return WebApiResponse.error(WebApiResponse.ResponseMsg.EXCEPTION.getValue());
        }
        if (pageInfo != null && pageInfo.getList() != null && pageInfo.getList().size() > 0)
        {
    ...
    @PostMapping("/create")
    public WebApiResponse createGoods(@RequestBody CreateGoodsReq createGoodsReq,
        @RequestHeader("Authorization") String token)
    {
        try
        {
    ...
    public WebApiResponse downGoods(@PathVariable String uuid, @RequestHeader("Authorization") String token)
    {
        ...
        try
        {
            ...
            String menberUuid = redisManager.get(token);
    ...
    public WebApiResponse deleteGoods(@PathVariable String uuid, @RequestHeader("Authorization") String token)
    {
        if (!NumberUtils.isDigits(uuid))
        {
            return WebApiResponse.error(WebApiResponse.ResponseMsg.PARAMETER_ERROR.getValue());
        ...
        try
        {
            result = goodsService.deleteGoods(uuid, token);
        }
        catch (BizException e)
        {
            LOG.error("/goods/upGoods/{uuid}:" + e);
        ...
        @RequestBody @ApiParam(value = "每日点击量", required = true) List<GoodsClickReq> goodsClickReq)
    {
        ...
        catch (Exception e)
        {
            ...
            result = WebApiResponse.error("增加商品点击量失败");
            LOG.error(e.toString());
        }
    ...
    public WebApiResponse queryMaterielAndStyle()
    {
        ...
        catch (Exception e)
        {
            ...
            result = WebApiResponse.error("查询风格和材质 失败");
            LOG.error(e.toString());
        }
    ...
    public WebApiResponse selectBuyersShowList(@RequestBody BuyersShowReq request,
        @RequestHeader("Authorization") String token)
    {
    ...
    @ApiOperation(value = "删除试衣记录")
    @RequestMapping(value = "/fitting/del/{fittingUuid}", method = RequestMethod.DELETE)
    public WebApiResponse delFitting(@PathVariable String fittingUuid, @RequestHeader("Authorization") String token){


        try {

            return goodsService.delFitting(fittingUuid, token);

        }catch (Exception e){
            LOG.error("-------------删除试衣记录 失败: createFitting:" + e + "-------------");
            return WebApiResponse.error(WebApiResponse.ResponseMsg.EXCEPTION.getValue());
        }
    }

    @ApiOperation(value = "新增试衣记录")
    @RequestMapping(value = "/fitting/add", method = RequestMethod.POST)
    public WebApiResponse createFitting(@RequestBody FittingRequest fittingRequest,
                                        @RequestHeader("Authorization") String token){
        WebApiResponse<?> result = null;

        try {

            result = goodsService.createFitting(fittingRequest, token);

        }catch (Exception e){
            LOG.error("-------------新增试衣记录 失败: createFitting:" + e + "-------------");
            return WebApiResponse.error(WebApiResponse.ResponseMsg.EXCEPTION.getValue());
        }
    ...
    @ApiOperation(value = "试衣记录列表")
    @RequestMapping(value = "/fitting/list", method = RequestMethod.POST)
    public WebApiResponse queryFitting(@RequestBody FittingRequest.Query query,
                                       @RequestHeader("Authorization") String token){
        ...
        try {
            ...
            result = goodsService.fittingList(query, token);

        }catch (Exception e){
            LOG.error("-------------新增试衣记录 失败: queryFitting:" + e + "-------------");
            return WebApiResponse.error(WebApiResponse.ResponseMsg.EXCEPTION.getValue());
        }
    ...
    @ApiOperation(value = "检查试衣记录是否有更新")
    @RequestMapping(value = "/fitting/notify", method = RequestMethod.POST)
    public WebApiResponse checkUpdateFitting(@RequestBody FittingRequest.CheckQuery query,
                                             @RequestHeader("Authorization") String token){
        try {

            return goodsService.checkUpdateFitting(query, token);

        }catch (Exception e){
            LOG.error("-------------检查试衣记录是否有更新 失败: createFitting:" + e + "-------------");
            return WebApiResponse.error(WebApiResponse.ResponseMsg.EXCEPTION.getValue());
        }
    }
