...
public class OpusServiceImpl implements OpusService
{
    ...
    public OpusDto queryOpusInfo(String uuid)
    {
        ...
        String cacheKey= CacheKeysConstants.generateCacheKey(OpusDto.class, uuid);
        ...
        else
        {
            ...
            if (expire <= 0)
            {
                ...
                redisManager.set(cacheKey, JSON.toJSONString(opusDto), TimeUnit.DAYS);
            }
        }
        if(opusDto == null)
        {
    ...
    public int updateOpus(Opus opus)
    {
        ...
        if(result != 0){
            //更新消息后更新缓存信息
            String cacheKey= CacheKeysConstants.generateCacheKey(OpusDto.class, opus.getUuid());
            long expire = redisManager.getExpire(cacheKey);
            if (expire <= 0)
            {
