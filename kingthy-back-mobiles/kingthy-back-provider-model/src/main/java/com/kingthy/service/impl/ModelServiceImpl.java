...
public class ModelServiceImpl implements ModelService
{
        ...
        throws IOException
    {
        ...
        figure.setStatus(Figure.Status.finish.getValue());
    ...
    public CreaterModelRes renderModel(CreaterModelReq createrModelReq)
    {
        ...
        try
        {
            ...
            if(resultCode == 0)
            {
                ...
                String cmdBd = commandBd+" "+commandSetting.getStandardModel()+" "+createrModelReq.getModelPath() + ".obj"+" "+createrModelReq.getModelPath() + ".bd "+createrModelReq.getModelPath() + ".dm";
                ...
                if(resultCodeBd == 0)
                {
                    ...
                    if (null != resultModelData && null != resultModelData.getFileUrl())
                    {
                        ...
                        {
                            LOGGER.info("bd文件上传成功:" + resultModelData.getFileUrl());
                            //2.上传bd文件到服务器
                            EncryptUploadDTO resultDmData = uploadFile(createrModelReq.getModelPath() + ".dm",createrModelReq.getMemberUuid()+".dm");
                            if(null != resultDmData && null != resultDmData.getFileUrl())
                            {
                                createrModelRes.setObj(JSON.toJSONString(resultModelData));
                                createrModelRes.setBd(JSON.toJSONString(resultBdData));
                                createrModelRes.setDm(JSON.toJSONString(resultDmData));
                                return createrModelRes;
                            }
                        }
                    }
