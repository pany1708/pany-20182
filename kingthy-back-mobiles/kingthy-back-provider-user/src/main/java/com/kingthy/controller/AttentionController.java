...
public class AttentionController 
{
    ...
    public WebApiResponse<?> getAttentionList(@RequestHeader(CommonUtils.REQUEST_HEADER_PARAMETER) String token)
    {
        try
        {
            ...
            LOG.error("getAttentionList error",e);
    ...
    @ApiOperation(value = "获取我的关注列表", notes = "获取我的关注列表")
    @PostMapping("/pageGetAttentionList")
    public WebApiResponse<?> pageGetAttentionList(@RequestHeader(CommonUtils.REQUEST_HEADER_PARAMETER) String token,
                                                  @RequestBody @ApiParam(name = "req", value = "分页查询参数") AttentionPageReq req)
    {
        try
        {
            //获取登录用户
            ...
            PageInfo<AttentionMemberDTO> pageInfo  = attentionService.pageGetAttentionList(req);
            return WebApiResponse.success(pageInfo);
        } catch (HystrixRuntimeException e){
            LOG.error("pageGetAttentionList error",e);
            if (e.getFailureType()== HystrixRuntimeException.FailureType.TIMEOUT ) {
                return WebApiResponse.error(WebApiResponse.ResponseMsg.TIMEOUT.getValue());
            }
            return WebApiResponse.error(WebApiResponse.ResponseMsg.HYSTRIX_EXCEPTION.getValue());
        }catch (Exception e) {
            LOG.error("pageGetAttentionList error:" + e.toString());
            return WebApiResponse.error(ResponseMsg.EXCEPTION.getValue());
        }
    }
    ...
    @ApiOperation(value = "也查询粉丝列表")
    @PostMapping("/pageGetFansList")
    public WebApiResponse<?> pageGetFansList(@RequestHeader(CommonUtils.REQUEST_HEADER_PARAMETER) String token,
                                                  @RequestBody @ApiParam(name = "req", value = "分页查询参数") AttentionPageReq req)
    {
        try
        {
            ...
            req.setMemberUuid(memberUuid);
            PageInfo<FansDTO> pageInfo = attentionService.pageGetFansList(req);
            return WebApiResponse.success(pageInfo);
        } catch (HystrixRuntimeException e){
            LOG.error("pageGetFansList error",e);
            if (e.getFailureType()== HystrixRuntimeException.FailureType.TIMEOUT ) {
                return WebApiResponse.error(WebApiResponse.ResponseMsg.TIMEOUT.getValue());
            }
            return WebApiResponse.error(WebApiResponse.ResponseMsg.HYSTRIX_EXCEPTION.getValue());
        }catch (Exception e) {
            LOG.error("pageGetFansList error:" + e.toString());
            return WebApiResponse.error(ResponseMsg.EXCEPTION.getValue());
        }
    }
