...
public class RabbitmqConfig implements RabbitListenerConfigurer{
    ...
    public Queue localCacheQueue(){
        ...
        return new Queue(RabbitmqConstants.LOCAL_CACHE_QUEUE);
    }
    @Bean
    public TopicExchange localCacheExchange(){

        return new TopicExchange(RabbitmqConstants.LOCAL_CACHE_EXCHANGE);
    }
    @Bean
    public Binding localCacheBindingQueueToExchange() {

        return BindingBuilder.bind(localCacheQueue()).to(localCacheExchange()).with(RabbitmqConstants.LOCAL_CACHE_ROUTING);
    }
    /**********本地缓存队列 end******************/
