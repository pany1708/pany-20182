...
import static com.kingthy.unionpay.sdk.SdkConstants.UNIONPAY_CNNAME;
import static com.kingthy.unionpay.sdk.SdkUtil.isEmpty;
...
public class CertUtil
{
    ...
    /** 商户私钥存储Map */
    private final static Map<String, KeyStore> KEY_STORE_MAP = new ConcurrentHashMap<String, KeyStore>();
...
//        RSA签名类型
        String rsaSign = "01";
        if (!rsaSign.equals(SdkConfig.getConfig().getSignMethod()))
        {
            LogUtil.writeLog("非rsa签名方式，不加载签名证书。");
            return;
        }
        if (SdkConfig.getConfig().getSignCertPath() == null || SdkConfig.getConfig().getSignCertPwd() == null
            || SdkConfig.getConfig().getSignCertType() == null)
        {
            LogUtil.writeErrorLog("WARN: " + SdkConfig.SDK_SIGNCERT_PATH + "或" + SdkConfig.SDK_SIGNCERT_PWD + "或"
...
//        rsa签名类型
    ...
    public static PrivateKey getSignCertPrivateKey()
    {
        try
        {
            ...
            PrivateKey privateKey =
                (PrivateKey)keyStore.getKey(keyAlias, SdkConfig.getConfig().getSignCertPwd().toCharArray());
            return privateKey;
        }
    ...
    public static PrivateKey getSignCertPrivateKeyByStoreMap(String certPath, String certPwd)
    {
        ...
        try
        {
            ...
            char[] password = null != certPwd ? certPwd.toCharArray() : new char[] {};
            PrivateKey privateKey = (PrivateKey)KEY_STORE_MAP.get(certPath).getKey(keyAlias, password);
            return privateKey;
        }
        catch (KeyStoreException e)
        {
...
//            证书数组位置
            int length = 2;
            if (tSplitStr != null && tSplitStr.length > length && tSplitStr[length] != null)
            {
                tPart = tSplitStr[length];
            }
        }
        return tPart;
    }
